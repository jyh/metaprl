 ...
 *)
   ...
   let rec expand_ext dforms proof node =
      match node with
       ...
       | RuleBox { rule_expr = expr;
            ...
            let goal =
               try snd (TacticInternal.refine (tac ()) t) with
   ...
   let proof_of_io_proof raw_attributes sentinal parse eval proof =
      let rec make_proof
   ...
   let proof_of_io_proof args sentinal parse eval proof =
      if old_io_proof_hack proof then
         proof_of_io_proof args sentinal parse eval ((Obj.magic proof) : old_proof)
      else
         ProofTerm_io.of_term args sentinal parse eval proof

   (*
    * Convert to an IO proof.
    *)
   let io_proof_of_proof = ProofTerm_io.to_term

   (*
    * Term conversions.
    *)
   let to_term = io_proof_of_proof
   let of_term = ProofTerm_io.of_term
   ...
   let term_of_io_proof parse eval proof =
      let proof =
         if old_io_proof_hack proof then
            ...
            proof
      in
         ProofTerm_io.revert proof

   let io_proof_of_term parse eval term =
      ProofTerm_io.convert term
   ...
   (*
    * Some basic operations allowed on terms.
    *)
   let status_of_io_proof proof =
      if old_io_proof_hack proof then
         StatusBad
      else
         translate_status (ProofTerm_io.status_of_term proof)

   let node_count_of_io_proof proof =
      if old_io_proof_hack proof then
         (-1, -1)
      else
         ProofTerm_io.node_count_of_term proof
