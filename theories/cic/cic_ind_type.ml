...
prim collapse_base {| intro [] |} :
	sequent { <H> >- 'C } -->
	sequent { <H> >- sequent [cic] { >- 'C } } = it

prim collapse_step {| intro [] |} :
	sequent { <H>; x:'T >- sequent [cic] { <J['x]> >- 'C['x] } } -->
	sequent { <H> >- sequent [cic] { x: 'T; <J['x]> >- 'C['x] } } = it
...
dform req3_m_df : sequent [|req3_m|] { <Hi> >- sequent [cic] { <Hc<| |> > >- it } } =
