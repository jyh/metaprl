...
@comment{@subsubsection["dce"]{Dead code elimination}}
@comment{JYH: this went below the following paragraph.
Complete elimination of dead-code is undecidable: for
...
change the behavior of the program $p$.  We approximate this with the following rewrite rules.
$$
@begin[array,l]
...
@comment{Furthermore, note that dead-code elimination of this form is aggressive.  For example, suppose we
...
@comment{@subsubsection["constant-folding"]{Constant-folding}}
@bf{Constant folding}
Another simple class of optimizations is constant folding.  If we have an expression that includes
only constant values, the expression may be computed at compile time.  In the following rewrites
the notation $@semleft @it{op} @semright$ is the
interpretation of the arithmetic operator in the meta-language.
$$
@begin[array,l]
