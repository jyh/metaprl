 ...
 *)
prim rfunctionElimination {| elim_resource [ThinOption] |} 'H 'J 'f 'a 'y 'v :
   [wf] sequent [squash] { 'H; f: { g | x:'A -> 'B['g; 'x] }; 'J['f] >- 'a = 'a in 'A } -->
...
let eqcd_rfunction_lambdaT p =
   let _, f, _ = dest_equal (Sequent.concl p) in
   let v, _ = dest_lambda f in
   let v = maybe_new_vars1 p v in
      rfunction_lambdaEquality (Sequent.hyp_count_addr p) v p

let eqcd_rfunction_applyT t p =
