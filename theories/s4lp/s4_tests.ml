...
define unfold_kwh {| reduce |} : kwh[i:n]{'a} <--> box[i:n]{'a} or box[i:n]{not{'a}}

define unfold_kao {| reduce |} : KAO <-->
	kwh[1]{m2} & kwh[1]{m3} &
	kwh[2]{m1} & kwh[2]{m3}

define unfold_w0 {| reduce |} : w0 <--> box[0]{KAO} & box[0]{not{not{m1} & not{m2} & not{m3}}}
define unfold_w2 {| reduce |} : w2 <--> w0 and box[0]{not{kwh[1]{m1}}} and box[0]{not{kwh[2]{m2}}}
...
define unfold_mch2_KAO {| reduce |} : mch2_KAO <-->	box[0]{kwh[1]{c2} & kwh[2]{c1}}

define unfold_s2_0 {| reduce |} : s2_0 <--> c1 & not{c2} & mch2_KAO

define unfold_s2_1 {| reduce |} : s2_1 <--> box[0]{not{kwh[1]{c1}} & not{kwh[2]{c2}}}

define unfold_s2_2 {| reduce |} : s2_2 <--> box[0]{c1 or c2}

define unfold_s2_3 {| reduce |} : s2_3 <--> box[0]{box[1]{c1}}
...
define unfold_mch3_KAO {| reduce |} : mch3_KAO <-->
	box[0]{
		kwh[1]{c2} & kwh[1]{c3} &
		kwh[2]{c1} & kwh[2]{c3}
	}

define unfold_s3_0 {| reduce |} : s3_0 <--> c1 & not{c2} & not{c3} & mch3_KAO

define unfold_s3_1 {| reduce |} : s3_1 <--> box[0]{not{kwh[1]{c1}} & not{kwh[2]{c2}} & not{kwh[3]{c3}}}

define unfold_s3_2 {| reduce |} : s3_2 <--> box[0]{c1 or c2 or c3}

define unfold_s3_3 {| reduce |} : s3_3 <--> box[0]{box[1]{c1}}
...
define unfold_mch4_KAO {| reduce |} : mch4_KAO <-->
	box[0]{
...
define unfold_S {| reduce |} : S <-->
	kwh[1]{c1} or kwh[2]{c2} or kwh[3]{c3} or kwh[4]{c4}

define unfold_NBK {| reduce |} : NBK <-->
	box[0]{
		not{kwh[1]{c1}} & not{kwh[2]{c2}} & not{kwh[3]{c3} & not{kwh[4]{c4}}}
	}

define unfold_s0 {| reduce |} : s0 <--> c1 & c2 & not{c3} & not{c4}
define unfold_s1 {| reduce |} : s1 <--> box[0]{c1 or c2 or c3 or c4}
define unfold_s2 {| reduce |} : s2 <-->
	box[0]{
		(c1 & c2) or
      (c1 & c3) or
      (c1 & c4) or
