 ...
 * removeHiddenLabelT repeatForT repeatMT repeatT selT
 * seqOnSameConclT seqT tactical thenAT thenMT thenT thenWT timingT
 ...
 * @begin[doc]
 * @begin[description]
 * @item{@tactic[repeatT], @tactic[repeatForT];
 * The (@tt{repeatT} $@i{tac}$) tactic applies the argument $@i{tac}$
 * repeatedly to the current goal and all of the generated subgoals until
 * 1) an application of $@i{tac}$ fails, or 2) the proof is complete (there
 * are no subgoals).  The (@tt{repeatForT} $i$ $@i{tac}$) repeats the application
 * at most $i$ times.}
 * @end[description]
 *
 * @docoff
 ...
 * (@tt{ifLabT} $l$ $@i{tac}$) applies the tactic $@i{tac}$ only if
 * the current goal has label $l$.}
 *
 * @item{@tactic[thenMT], @tactic[thenAT], @tactic[thenWT];
 * The (infix) @tt{thenMT}, @tt{thenAT}, and @tt{thenWT} are like the
 * @tt{thenT} tactical, except that they apply their second argument
 * only to the goals labeled ``main'', ``antecedent'', or ``wf'' respectively.}
 *
